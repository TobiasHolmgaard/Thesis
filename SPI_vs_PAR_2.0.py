...
# selected_SPI_data = selected_SPI_data.where(selected_SPI_data < 0)
...
# selected_PAR_data = selected_PAR_data.where(selected_SPI_data < 0)
...
for lat in range(len(spi_monthly_array['latitude'])):
    for lon in range(len(spi_monthly_array['longitude'])):
        spi_data = selected_SPI_data[:,lat,lon].where(selected_SPI_data[:,lat,lon] <= -0.5, drop = True).values
        # spi_data[spi_data == np.inf] = np.nan
        par_data = selected_PAR_data[:,lat,lon].where(selected_SPI_data[:,lat,lon] <= -0.5, drop = True).values
        ...
        if np.all(np.isnan(par_data)) ==  True:
            ...
            polifit = poly.polyfit(spi_data[:], par_data[:],1)
