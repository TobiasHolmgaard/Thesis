# -*- coding: utf-8 -*-
"""
Created on Mon Jan 22 10:52:26 2024
...
Two_periods = [[0,360], [516,876]]
perioder = ['1950-1979', '1993-2022']
q = 0
for period in Two_periods:
    
    time_spi = E_obs['time'][period[0]:period[1]]
    longitude = E_obs['longitude']
    latitude = E_obs['latitude']
    precip = E_obs['rr'][period[0]:period[1]]
    ...
    ax.set_title('Std for precipitation in gridcells ' + perioder[q], fontsize=25)
    plt.show()
    ...
    colorbar = plt.colorbar(mappable=std_precip.plot.contourf(ax=ax, levels = clevs, 
                            ...
                            ax=ax, orientation='vertical', pad=0.02)
    colorbar.set_label('standard deviation (mm)')
    ax.set_title('Std for precipitation in gridcells ' + perioder[q] +' - masked', fontsize=20)
    plt.show()
    ...
    colorbar = plt.colorbar(mappable=std_norm_precip.plot.contourf(ax=ax, levels = clevs, 
                            ...
                            ax=ax, orientation='vertical', pad=0.02)
    colorbar.set_label('Std / mean (mm)')
    ax.set_title('Std normalized precip in gridcells '+ perioder[q] +' - masked', fontsize=20)
    plt.show()
